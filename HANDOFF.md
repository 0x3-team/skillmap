...
Merged release candidate: `67129297d08f7f7bc88800015b336a2a7bb1b139`
Current merged `main`: `29a356a9b809d29ff8c986fbd5a0af78d87e479c`
Release tree: `3a70dbafca99153ad80d67601a5b2e3bbc2d47d5`
Current status: the free-public-alpha candidate was locally validated, pushed, accepted by the required scoped remote CI, and merged into canonical `main` with the identical release tree. Gitea candidate run `44` passed, GitHub Actions run `29285742074` job `86937705880` passed the JIT `hosted-web` scope, and post-merge Gitea `main` run `47` passed. The frozen static receipt is `sha256:3dd68b69f5faad0e6cf70e03dbf98cedb735ed5661dc2c6a8d01c799ed7b2996`; the frozen database receipt is `sha256:ada2c9d819dce02a3b89971c44119eb96ef89f244ccd692439e80281f64056d1`. Those receipts prove source integration and only their recorded CI scopes. The product is not deployed or verified live, and no remote database/web project, OAuth path, backup destination, worker schedule, domain, public indexing, or open-user launch is claimed. Launch remains `NO-GO`.

Current continuation boundary: the Unreleased worktree also contains a service-role-only, bounded, redacted submission summary/list/detail read plane and corrected operator commands. The raw generated Supabase types remain byte-exact for schema parity, and the web application uses a narrow override for nullable return fields from the three operator `RETURNS TABLE` RPCs. Its local database, CLI, root, web, package, and adversarial evidence must be bound to a later candidate/merge ledger row before the baseline source-integration receipt above can be treated as covering it.
...
Current surface boundary: `apps/web` contains a locally validated Supabase-backed catalog, free-account saved-skill flow, exact-commit submission/status/withdrawal, current-version public audit and provisional-grade projections, authenticated suspicious-listing reports, export, and self-deletion alongside the separate recorded-fixture/redacted-snapshot dashboard. A constrained server-only worker can claim one queued submission, fetch bounded inert public GitHub bytes, emit static-audit and letterless provisional-grade receipts, complete/requeue it, and publish reviewed metadata through service-only RPCs. A separate service-only read plane summarizes and cursor-pages the queue and returns bounded redacted exact receipt history without claiming or mutating rows; its application type boundary corrects nullable RPC return fields without weakening raw generated-type parity. Report disposition and receipt-backed deprecate/quarantine/revoke/restore controls are also implemented locally. The package contains `assets/local-app/v1`, served only by the foreground `skillmap dashboard` loopback connector. The baseline hosted-workflow source is pushed, merged, and accepted by scoped remote CI; later Unreleased work needs its own release-ledger receipt. No hosted path is claimed provisioned, deployed, scheduled, or verified live. Team sync, billing, package mirroring/loading, TUF distribution, advanced hosted routing, and current-letter behavioral grading remain unimplemented or deliberately deferred.
...
CI receipt note: Gitea feature run 18 passed `1427e277e46315de5792a973deded1af4c274195`, and main run 19 passed `f9ea0fa0d9711b5b0a61d24555ed9102fff20eb3`; those commits share historical tree `be96e2a71f2b38ded52ac6e1077ebbcd1dc0bbc1`. Historical hosted-foundation feature run 14 passed `00e29a442b3ef03345f25970aa2abff4655d259d`, and main run 15 passed `295dffe031d3010bb241ade75e9f249c97cd6063`; those receipts do not cover the current candidate. The current candidate receipt is Gitea run `44` plus GitHub Actions run `29285742074`, JIT `hosted-web` job `86937705880`, against candidate `67129297d08f7f7bc88800015b336a2a7bb1b139`; the identical tree was squash-merged as `main` commit `29a356a9b809d29ff8c986fbd5a0af78d87e479c`, and post-merge Gitea run `47` passed. GitHub acceptance is scoped to that hosted-web job and is not a whole-repository, deployment, or live-product receipt.
