...
Current status: canonical `main` still contains the previously accepted hosted-foundation anchor described below. The free-public-alpha implementation is on `codex/hosted-library-foundation`; its exact release input is the current reviewed tracked tree after the intended launch-readiness changes are committed, while user-owned `.gitignore`, `.chunk/`, `.claude/`, and `.codex/` state remains excluded. That frozen tree must be squashed onto a fresh branch whose direct parent is the re-fetched canonical `main`, then pass clean-candidate and remote checks. It is not yet claimed pushed, reviewed remotely, deployed, or verified live. The last recorded GitHub Actions state was spending-blocked before execution, but that state is drift-prone and must be rechecked rather than assumed. Public release requires both current Gitea authority lanes and the exact-commit GitHub `hosted-web-browser` lane; inability to run either remains `NO-GO`. No remote database/web project, OAuth path, backup destination, worker schedule, domain, or public indexing state is claimed by this handoff.
