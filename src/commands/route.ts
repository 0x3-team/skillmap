...
export async function routeCommand(cwd: string, positionals: string[], flags: Record<string, string | boolean | string[]>): Promise<unknown> {
  ...
  if (hasFlag(flags, 'trace')) return { result, statusWarnings: status.warnings, trace: renderTrace(result, status.warnings) };
  return { ...result, statusWarnings: status.warnings };
}
...
function renderTrace(result: ReturnType<typeof routePrompt>, warnings: string[]): string {
