...
Do not start a worker or service-role operator command until `20260727050300_operator_submission_read_plane.sql`, `20260727050305_atomic_report_enforcement.sql`, `20260727050312_github_provider_rate_limit_deferral.sql`, `20260727050316_report_authorization_enforcement.sql`, `20260727050320_operator_dual_control.sql`, `20260727050324_hosted_evidence_version_authority.sql`, `20260727050328_hosted_report_idempotency_recovery.sql`, and every preceding migration are applied and the exact candidate has passed database lint, full pgTAP, generated `api,private` type parity, the worker typecheck, and the web application typecheck. The raw `database.types.ts` stays byte-exact for `api,private` generator comparison; `database.runtime.types.ts` excludes `private` from application clients and corrects nullable return fields for only the three operator `RETURNS TABLE` RPCs. `process-once` peeks one exact candidate, checks an unauthenticated GitHub core budget with a two-request shared-egress reserve, claims only that candidate, and unconditionally renews a successful claim through `api.renew_skill_submission_claim`. A raced primary or secondary provider limit returns the exact claim to `queued`, retains a bounded retry time, and refunds the audit attempt without a worker-run row. Claim, completion, retained evidence, and publication require the exact migration-owned worker/audit/grade tuple. Publication also requires claim-scoped exact license evidence, current publisher authorization, and target-bound collision authority. Report intake independently requires that same current authorization; owner-safe retry recovery binds the exact request UUID before falling back to the current queued target; report confirmation requires one atomic exact-version quarantine or revocation; and both operator queues use paired cursors. Consequential publisher-authorization, collision-review, publication, lifecycle, and report-disposition commands require a short-lived exact-envelope approval from an approver and execution by a distinct executor. The atomic-enforcement migration fails closed if the target contains legacy resolved reports; reconcile those exact targets through a reviewed forward migration instead of bypassing the guard. Operator-before-migration is a hard `NO_GO`, not a recoverable compatibility mode. Follow [the free public alpha runbook](../../docs/operations/free-public-alpha-runbook.md#hard-worker-migration-gate) before the first run and after every database deploy.
