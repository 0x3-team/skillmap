...
import { authenticateAccessToken } from "@/lib/device-auth/lifecycle-service.server";
import { AUTHENTICATE_PATH } from "@/lib/device-auth/lifecycle-contracts.server";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    assertNoQuery(url);
    if (url.pathname !== AUTHENTICATE_PATH) throw new DeviceAuthError("proof_invalid");
    ...
    const result = await authenticateAccessToken({ repository, lookupKeys: deviceAuthLookupKeysFromEnvironment() }, {
      body, rawBody,
      proof: { configuredOrigin: cfg.verificationUrl, path: AUTHENTICATE_PATH, method: "POST", proofSuite, audience, purpose,
