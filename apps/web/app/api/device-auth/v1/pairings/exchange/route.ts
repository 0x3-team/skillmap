...
import { EXCHANGE_PATH, isExchangeRequest } from "@/lib/device-auth/poll-exchange-contracts.server";
import { exchangePairing } from "@/lib/device-auth/poll-exchange-service.server";
import { deviceAuthTokenCryptoFromEnvironment } from "@/lib/device-auth/poll-exchange-crypto.server";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (!isExchangeRequest(body)) throw new DeviceAuthError("invalid_request");
    ...
    if (url.pathname !== EXCHANGE_PATH) throw new DeviceAuthError("proof_invalid");
    ...
    const result = await exchangePairing(repository, body, {
      configuredOrigin: cfg.verificationUrl, path: EXCHANGE_PATH, proofSuite: proofSuiteHeader,
      audience: body.audience, purpose: "exchange", proofNonce, issuedAt, bodySha256,
      signature, proofSuiteHeader, audienceHeader, purposeHeader,
      deviceIdHeader, idempotencyKey
    }, deviceAuthTokenCryptoFromEnvironment(), proofKey);
