...
type ErrorObject = { instancePath?: string; schemaPath: string; keyword: string; message?: string };
...
import { effectiveRegistryUsesFixtureState } from './fixture-path';
import { rankRoutePrompt, validateRoutePrompt, type RouteRankingSkill } from './route-ranking';
import { CONTRACT_SCHEMAS, type ContractSchemaId } from './schema-bundle';
import { CONTRACT_STANDALONE_VALIDATORS } from './standalone-validators';
import type { ContractBySchemaId, EvalRunV3, KnownContractSchemaId, RevisionRef, Sha256Digest } from './types';
...
function validateContractInternal(schemaId: string, value: unknown, context?: SemanticValidationContext): ContractValidationResult {
  ...
  const issues = valid ? [] : formatValidationIssues(validator.errors);
  if (valid) {
    try {
      canonicalJson(value);
    } catch {
...
function formatValidationIssues(errors: ErrorObject[] | null | undefined): ContractIssue[] {
