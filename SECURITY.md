...
The separate hosted trust alpha supports metadata-only catalog browsing, free accounts and saved skills, exact-commit submissions, inert static audit, letterless provisional grading, operator publication, and authenticated suspicious-listing reports. Confirmed reports atomically quarantine or revoke the exact reported version. That plane is governed by explicit Supabase grants and forced RLS, contains no raw prompts or private local skill bodies, defaults to no indexing, and is live only when its deployment ledger records the exact commit and acceptance evidence. It is not yet a public beta and is distinct from the local connector.
...
- SkillMap does not provide team tenancy, cloud synchronization, package execution, current letter grading, or hosted routing in this alpha.
