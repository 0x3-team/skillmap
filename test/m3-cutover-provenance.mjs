...
test('historical M3.02 receipt is immutable and superseding receipt binds current artifacts', () => {
  ...
  const m4SupersededArtifactPaths = new Set([
  ...
  for (const artifact of receipt.bounded_artifacts) {
    ...
    // M4 adds the managed-manifest contract and regenerates both checked-in
    // validator chains. Keep the immutable M3 receipt as historical evidence;
    // current generated bytes are covered by the generator convergence and M4
    // contract acceptance tests.
    if (!m4SupersededArtifactPaths.has(artifact.path)) {
  ...
  const m4SupersededCloudflareArtifactPaths = new Set([
    'apps/web/package.json',
    'apps/web/tests/hosted-boundaries.test.mjs'
  ]);
  for (const artifact of cloudflareRuntimeReceipt.artifacts) {
    // M4 adds package scripts and a monorepo build-boundary assertion after the
    // deployed M3 receipt. Keep those historical hashes as M3 evidence; current
    // bytes are covered by package lifecycle, hosted-boundary, and production
    // build gates.
    if (m4SupersededCloudflareArtifactPaths.has(artifact.path)) continue;
