...
| MCP-010 | Add redacted structured telemetry: request-scoped random ID, method/tool, status, duration/CPU, version, index digest, and cache outcome; enqueue it after response with waitUntil or a bounded sink. Do not emit a user-, seat-, account-, token-, or client-stable subject identifier. | MCP-003 | No prompt, skill body, token, email, cookie, full IP, or participant-linkable identifier is logged; request IDs are random and non-linkable; telemetry failure does not delay or fail a successful recommendation. |
...
| DEP-006 | Process only owned/authorized rehearsal skills through normal submission, audit, provisional grade, collision, authorization, dual-control publication, and edge export. Derive a unique operation ID from source revision plus publication digest, enforce it in the database/export ledger, and promote the immutable edge pointer only with compare-and-swap against the expected prior digest. | DEP-002, DEP-004, DATA-001 | No hand-created or duplicate rows; exact retries return the existing result; partial database/export commits reconcile before pointer promotion; Supabase and edge digests match. |
| DEP-007 | Install manual one-shot operator workflow or reviewed scheduler; prove lease, digest-plus-source-revision idempotency, retry, provider deferral, DLQ, alert, and partial-commit reconciliation behavior. A replay may resume missing immutable writes but cannot duplicate publication or promote an inconsistent edge version. | DEP-006 | Operator execution receipt; replay and timeout-after-commit fixtures pass; worker remains outside request path. |
