...
The local-sensitive archival bundle requires `--include-sensitive-local`, an explicit output inside `.skillmap/private-exports/`, realpath containment, and mode `0600` on Linux or macOS. It includes config, opaque identity, exact policy-v2 pointer/revision/migration/rollback files, and the legacy artifact set. It is marked local-sensitive/non-shareable, cannot be combined with dashboard export, and imports remain archive-only rather than automatically restoring state. This sensitive export fails closed before creating a target on Windows because Node's POSIX mode bits do not establish a private Windows ACL; use the default redacted export there, or create the archive from a protected Linux/macOS workspace.
