...
- Local-sensitive archival export requires the explicit `--include-sensitive-local` flag and an output confined by realpath to `.skillmap/private-exports/`. It includes identity and exact policy-v2 pointer/revision/migration/rollback state as well as legacy artifacts. On Linux and macOS these files are mode `0600`, marked `local-sensitive`, archive-only on import, and must never be shared or used by the dashboard/sync path. The command fails closed before creating a target on Windows because POSIX mode bits do not establish a private Windows ACL.
...
- `skillmap mcp` is a local metadata/routing surface in v1. Five tools are strictly read-only. `route_prompt` also records the existing bounded, prompt-free route event, so its SDK annotation is non-destructive but not falsely idempotent/read-only. No tool can return a skill body or path, update skills, install or execute code, audit or grade a skill, or mutate policy.
- The official MCP SDK owns lifecycle, initialized notifications, schema validation, list/call dispatch, and unknown-method behavior. SkillMap supplies a bounded line adapter before the SDK (64 KiB per request), a combined structured/text response budget (below 512 KiB), explicit redacted projections, safe error envelopes, and stdout reserved for protocol frames.
- Discovery indexes contain only deterministic approved-registry metadata, are keyed by effective revision digest, retain at most two revisions per runtime, and can be disabled for the local MCP process with `SKILLMAP_DISCOVERY_STRATEGY=reference`. `shadow` returns the scanner result and reports a digest-only mismatch to stderr. The full scanner remains the correctness oracle; no fuzzy, embedding, LLM, network, or skill-body data enters routing.
