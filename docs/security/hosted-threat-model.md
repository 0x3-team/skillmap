...
Phase 1 has anonymous catalog reads and account-owned saves only. It does not fetch third-party sources, mirror packages, execute skills, run graders, publish TUF metadata, accept publisher mutations, expose operator controls, or deploy remotely. Seeds are first-party MIT files pinned to an immutable commit and exact entrypoint digest, while provenance/audit/compatibility/grade remain explicitly unverified/not-run/not-tested/ungraded.
