...
select is(
  ...
  44::bigint,
  'the API security-definer boundary contains exactly forty-four reviewed functions'
);
select is(
  (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'api' and p.prosecdef and p.proname in (
      ...
      'device_auth_cancel_v1', 'device_auth_authenticate_v1', 'device_auth_authenticate_import_v1',
