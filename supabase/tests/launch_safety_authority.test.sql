...
select is((select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'api' and p.prosecdef), 17::bigint, 'API security-definer surface remains the exact seventeen-function allowlist');
